-- Impala (TODAY, 2 дня): детализация заказа из лёгкой crm2.crm2_crm_cust_order_day
...
--
-- ДЕДУП: один и тот же заказ (id) может встретиться в day-таблице в разных
-- состояниях (напр., вечером вчера открыт, сегодня закрыт). Берём строку с
-- максимальным update_date — самое свежее состояние.
SELECT
    o.id,
    o.customer_id,
    ...
    cast(o.update_date AS timestamp)          AS update_date,
...
FROM (
    SELECT o2.*,
           row_number() OVER (
               PARTITION BY o2.id
               ORDER BY cast(o2.update_date AS timestamp) DESC NULLS LAST
           ) AS rn
    FROM crm2.crm2_crm_cust_order_day o2
    WHERE o2.id IN ({{crm_order_id__ids}})
) o
